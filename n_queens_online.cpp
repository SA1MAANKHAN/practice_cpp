...
int main()
{
  ios_base::sync_with_stdio(false);
    cin.tie(NULL);
        ...
        cin >> n;
        if(n<4)
            cout << "N Queens only works for n equal to 4 or above. PLEASE ENTER A VALID INPUT" << endl;
        else
         for (int i = 0;i < n;i++) {
             for (int j = 0;j < n;j++) {
                 grid[i][j] = 0;
         ...
         bool res = solve(n, 0);
         if(res == false) {
             cout <<"" << endl; //if there is no possible solution
         } else {
             cout << endl;
         }
  cout << "number of solutions is :  " << solution << endl;
  return 0;
}
